...
export function ServiceProviderProfileCompleteForm() {
  ...
  const { updateServiceProviderProfile, isLoading } = useServiceProviderProfileUpdate();
  ...
  const nextStep = () => {
    if (isAllCurrentStepFieldFilled()) {
      ...
      toast({
        ...
        description: "Make sure you've completed all mandatory inputs for this step.",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <h1 className="text-2xl font-bold mb-1">Complete Your Service Provider Profile</h1>
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-xs text-gray-500">{((currentStep / 3) * 100).toFixed(0)}% completed</div>
        </div>

        {renderCurrentStep()}

        {formError && !debugMode && (
          <p className="text-red-500 text-sm mt-2">Please fill in all required fields before proceeding.</p>
        )}
      </div>
          ...
          >
            {isLoading ? (
              ...
              <span className="flex items-center">
                {debugMode ? "Test Submit" : "Submit"} <Check className="ml-2 w-4 h-4" />
              </span>
            )}
          </Button>
        )}
