...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      showpopUp({ type: "info", message: "Logging in..." });
      const res = await axios.post(`${API_HOST_URL}/api/v1/auth/login`, { email, password });
      ...
      if (check && authKey) {
        window.localStorage.setItem("NXGJOBHUBLOGINKEYV1", JSON.stringify({ authKey, email, id }));
      } else if (!check && authKey) {
        window.sessionStorage.setItem("NXGJOBHUBLOGINKEYV1", JSON.stringify({ authKey, email, id }));
      }
      ...
      showpopUp({ type: "danger", message: `Login failed, ${errorMessage}` });
      setTimeout(() => showpopUp(undefined), 5000);
    }
  };

  const AutoLoginUser = async () => {
    const storedData = JSON.parse(window.localStorage.getItem("NXGJOBHUBLOGINKEYV1"));
    if (storedData) {
      try {
        const userRes = await axios.get(`${API_HOST_URL}/api/v1/auth/get-user`, {
          headers: { "Content-Type": "application/json", authorization: storedData.authKey },
        });
        if (!userRes.data.userType) {
          navigate("/create");
        } else {
        ...
        // Optionally, remove the invalid token from storage
        ...
        <div className="form-col">
          ...
          <form className="logintech" onSubmit={handleLogin}>
            ...
            <div className="log-form">
              ...
              />
              <div className="forgot">
                <Link to="/forgotpassword" style={{ color: "#A39E9E", textDecoration: "underline" }}>
                  Forgot Password?
                </Link>
              </div>
              <div className="remember">
            ...
            <p className="signup">
              Don't have an account?{" "}
              <Link to="/register" style={{ color: "#2596BE", textDecoration: "underline" }}>
                Sign Up
              </Link>
            </p>
          </form>
